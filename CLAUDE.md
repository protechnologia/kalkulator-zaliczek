...
**Temperatura zewnętrzna** (jedna na miesiąc, klucz `"RRRR-MM"` w `P.temps`, pole obok ceny w pierwszej kolumnie M01) to **pomiar, nie taryfa**: BEZ carry-forward (`P.getTemp` zwraca `null` przy braku wpisu), 0 i wartości ujemne są poprawne. Czysto informacyjna — wchodzi tylko do wykresu M02 (metryka `temp`), nie do kosztów ani doboru zaliczek.
