...
- Otwórz [kalkulator-zaliczek.v1.2.html](kalkulator-zaliczek.v1.2.html) w przeglądarce — to wszystko. Brak buildu, brak `npm`, brak dependencji.
- Foldery `css/` i `js/` muszą leżeć obok pliku HTML.
- Brak lintera — weryfikacja UI jest ręczna w przeglądarce (DevTools console).
- **Samotest** (Node, bez frameworka): `node tests/hdd-selftest.js` — ładuje
  produkcyjne pliki JS przez shim `window` i sprawdza silnik sygnatury HDD
  (fit OLS, kwantyl, sanity klimatologii, ścieżka end-to-end przez `P.forecastGJ`)
  na wartościach kontrolnych. Exit code ≠ 0 przy porażce. Node służy TYLKO do
  testów i generatora klimatologii — aplikacja nadal działa bez niego.
...
2. **Sztywna kolejność ładowania skryptów** (zob. koniec pliku HTML):
   `config → climate → data → estimate → persist → vendor/exceljs → xlsx → render → render.m01..m04 → app`.
   Nowy plik JS musi dostać `<script>` we właściwym miejscu tej kolejności.
3. **Żadnych zewnętrznych bibliotek JS** — z **jednym wyjątkiem**: raport XLSX
   ([js/kz.xlsx.js](js/kz.xlsx.js)) używa zvendorowanej lokalnie kopii
   `vendor/exceljs.min.js` (ExcelJS 4.4.0, Apache-2.0; bundle UMD ładowany zwykłym
   `<script>`, wystawia global `window.ExcelJS`). To NIE łamie sensu zasady —
   biblioteka leży na dysku, działa z `file://`, bez CDN/fetch (zob. pkt 4).
   Wykresy nadal to ręcznie generowany SVG (zob. helpery `P._frame`,
   `P._smoothPath`, `P._niceMax` w [js/kz.render.js](js/kz.render.js)). Druga
   zewnętrzna zależność to font z Google Fonts (kosmetyka).
4. **Bez `cdn`/fetch w runtime** — całość działa offline z dysku (ExcelJS jest
   lokalną kopią, nie pobieraniem w locie).
...
| [js/kz.climate.js](js/kz.climate.js) | `P.CLIMATE` — klimatologia HDD per miasto (**plik GENEROWANY** przez `tools/hdd-climate.js`, nie edytować ręcznie; commitowany do repo) |
| [js/kz.data.js](js/kz.data.js) | Magazyny `records`/`prices`/`temps`/`advances`/`areas` + CRUD, klucze, `periodWindow`, `isPast` |
| [js/kz.estimate.js](js/kz.estimate.js) | Prognoza zużycia (trend liniowy + sygnatura energetyczna HDD dla CO), `simulate()` (dobór zaliczek na zakresie M01: ustalone z M03 + dobrany „ogon"), `metricMatrix(metric)` (wielkość + prognoza „ogona" dla M02) |
| [js/kz.persist.js](js/kz.persist.js) | Eksport/import JSON + best-effort autosave w `localStorage` |
| [js/kz.xlsx.js](js/kz.xlsx.js) | Raport XLSX (`P.exportXLSX`/`P.buildXLSX`): 2 arkusze „Zaliczki CO/CWU", wiersze = budynki, kolumny = miesiące (od początku trwającego okresu rozliczeniowego) ze **stawkami** zaliczek (CO [zł/m²], CWU [zł/m³]). Jedyny konsument `window.ExcelJS` |
| `vendor/exceljs.min.js` | Zvendorowana lokalnie kopia ExcelJS 4.4.0 (Apache-2.0, bundle UMD → global `window.ExcelJS`). Jedyna biblioteka JS w projekcie; ładowana przed `kz.xlsx.js` |
...
| [tools/hdd-climate.js](tools/hdd-climate.js) | Generator `js/kz.climate.js` (Node, uruchamiany ręcznie przez dewelopera — NIE jest częścią aplikacji) |
| [tests/hdd-selftest.js](tests/hdd-selftest.js) | Samotest silnika sygnatury HDD (Node, bez frameworka) |
...
**Temperatura zewnętrzna** (jedna na miesiąc, klucz `"RRRR-MM"` w `P.temps`, pole obok ceny w pierwszej kolumnie M01) to **pomiar, nie taryfa**: BEZ carry-forward (`P.getTemp` zwraca `null` przy braku wpisu), 0 i wartości ujemne są poprawne. Wchodzi do wykresu M02 (metryka `temp`) oraz do **sygnatury energetycznej HDD** (przy prognozie CO `'hdd'`: temperatury historyczne uczą model, a jawna temperatura przyszłego miesiąca nadpisuje klimatologię) — wtedy pośrednio wpływa też na koszt i dobór zaliczek w M04.

## Moduł 02 — zużycie (9 wielkości)

Wykres słupkowy dla **jednego budynku** (własny select `state.m02Building`) po zakresie miesięcy M01. Jeden select (`#kz-m02-metric`, stan `state.m02Metric`) wybiera jedną z 9 wielkości zdefiniowanych w `P.M02_METRICS` ([js/kz.config.js](js/kz.config.js)): CO‑wskaźnik [GJ/m²], CO‑zużycie [GJ], **CO‑koszt [zł]**, CWU‑wskaźnik [GJ/m³], CWU‑zużycie [GJ], CWU‑woda [m³], **CWU‑koszt [zł]**, **Cena ciepła [zł/GJ]**, **Temperatura zewn. [°C]**. Każda metryka ma `{ medium, field, label, unit }`, gdzie `field` to `'intensity'` (GJ/dzielnik), `'gj'`, `'qty'`, `'cost'` (GJ × cena), `'price'` (cena ciepła — wartość globalna z `P.prices`, niezależna od budynku/zużycia) lub `'temp'` (temperatura — wartość globalna z `P.temps`; może być ≤ 0, więc wykres używa `opts.yMin` w `P._frame`, rysuje słupki w dół od wyróżnionej linii zera; prognoza „ogona" **tylko przy metodzie CO `'hdd'`**: `P.tempForecast` odwraca aproksymację HDD — `T = tBase − HDD/dni` — i pokazuje temperaturę zakładaną przez sygnaturę, paź–kwi; maj–wrz i luki w historii zostają **celowo puste**, bo fallback trendem nie zakłada żadnej temperatury). Oś X = miesiące, jeden słupek na miesiąc, jeden kolor serii (`COLOR`), bez legendy.

**Sposób prognozy** wybierany **osobno per medium** (dwa selecty): **CO** (`#kz-m02-method-co`, stan `state.m02Method`) ma dwie pozycje: `'trend'` — „Trend per miesiąc (GJ)" (domyślnie) i `'hdd'` — „Sygnatura energetyczna (HDD)" (zob. niżej). **CWU** (`#kz-m02-method-cwu`, stan `state.cwuBasis`) ma dwie: `'intensity'` = „Trend per miesiąc (GJ/m³ i m³)" → `forecastGJ = trend(GJ/m³) × trend(m³)` (domyślnie; zob. `forecastIntensity` w [js/kz.estimate.js](js/kz.estimate.js)), `'gj'` = „Trend per miesiąc (GJ i m³)" → trend wprost na GJ. Oba sposoby to **globalne ustawienia** — zawsze aktywne (nie blokujemy ich przy „cudzej" metryce). **Wpływają na M04** (przestawiają `P.forecastGJ` → koszt i dobór zaliczek), dlatego listenery w app.js wołają pełne `P.update()`, nie sam `renderM02` (dotyczy też miasta i percentyla HDD). Stary plik JSON bez nowych kluczy stanu → `Object.assign` zachowuje domyślne wartości.

### Sygnatura energetyczna CO (`m02Method: 'hdd'`)

Model `E_m = a + b·HDD_m + c·t` (a — baza [GJ], b — czułość pogodowa [GJ/stopniodzień], c — trend międzysezonowy [GJ/sezon], t — numer sezonu), dopasowywany **jedną wspólną regresją OLS** (równania normalne + eliminacja Gaussa, `P._signatureFit`) na wszystkich miesiącach grzewczych budynku — NIE per miesiąc kalendarzowy. Wszystko liczone **po miesiącach**: `HDD_m = max(0, 15 − T_śr_mies) × liczba_dni` — tą samą aproksymacją, którą wygenerowano klimatologię, więc błąd przybliżenia znosi się między uczeniem a prognozą. Sezon grzewczy = **paź–kwi** (id = rok października); maj–wrz oraz każdy przypadek braku warunków → **cichy fallback na trend per miesiąc**.

**Sezon grzewczy NIE ogranicza prognozy ani zaliczek do paź–kwi.** Realne dane mają zużycie CO także w maju/wrześniu (dogrzewanie, ~10–30 GJ) i śladowe latem (straty, cyrkulacja): te miesiące prognozuje fallback trendem per miesiąc, a ich koszt normalnie wchodzi do `simulate()` i doboru stawek w M04 — sygnatura obsługuje tylko swoją część roku. Z **uczenia** sygnatury maj–wrz jest wykluczony celowo: zużycie tych miesięcy nie zależy od pogody (HDD≈0), więc psułoby regresję. Zakres paź–kwi jest zaszyty w **trzech sprzężonych miejscach**: `seasonOf()` w [js/kz.estimate.js](js/kz.estimate.js), `HEATING_MONTHS` w [tools/hdd-climate.js](tools/hdd-climate.js) i (pochodnie) klucze `monthlyHDD` w generowanym [js/kz.climate.js](js/kz.climate.js) — zmiana zakresu wymaga zmiany dwóch pierwszych naraz, regeneracji klimatologii i poprawki asercji „7 miesięcy" w samoteście.

- **Punkt uczący** = rekord CO **plus jawna temperatura** w `P.temps` (bez imputacji). Degradacja: ≥3 sezony → pełny model (a, b, c); mniej → `c = 0`; <5 punktów → `null` (fallback). Fit (`P.fitSignature`) liczony od nowa przy każdym wywołaniu — celowo bez cache (dane małe).
- **HDD prognozy** (`hddForecast`): jawna temperatura użytkownika ma pierwszeństwo (scenariusz „wiem lepiej", bez percentyla); inaczej `monthlyHDD[m] × k_klimat × ratio`, gdzie `k_klimat` ekstrapoluje trend sum sezonowych na sezon prognozy, a `ratio = kwantyl(seasonSums, P)/średnia` — percentyl surowości zimy liczony **na sumach sezonowych** (sumowanie percentyli miesięcznych zawyżałoby wynik).
- **Kontrolki** (drugi wiersz M02, aktywne tylko przy `'hdd'`): miasto `#kz-m02-city` (stan `state.hddCity`; opcje budowane z `Object.keys(P.CLIMATE)` w `fillHddControls()` — dodanie miasta nie wymaga edycji HTML) i percentyl `#kz-m02-hdd-p` (stan `state.m02HddP`: 50/70/80/90, domyślnie 80). Diagnostyka fitu (a, b, c, R², liczba miesięcy/sezonów) w nagłówku wykresu (`signatureInfo()` w [js/kz.render.m02.js](js/kz.render.m02.js)); przy braku fitu nagłówek mówi wprost o fallbacku.
...
## Klimatologia HDD (plik generowany)

[js/kz.climate.js](js/kz.climate.js) zawiera `P.CLIMATE` — per miasto: rok typowy `monthlyHDD` (7 miesięcy grzewczych), sumy sezonowe `seasonSums`, średnią i trend. Źródło: archiwum Open-Meteo (średnie dobowe temperatury), agregowane do średnich miesięcznych i przeliczane na HDD przy bazie 15 °C. Plik jest **commitowany** (aplikacja działa offline), a regeneruje go deweloper:

**Dodanie nowego miasta:**
1. Dopisz wiersz do `CITIES` w [tools/hdd-climate.js](tools/hdd-climate.js) (id, nazwa, współrzędne).
2. Uruchom `node tools/hdd-climate.js` — przepisuje cały `js/kz.climate.js`.
3. Sanity: średnia suma sezonowa dla miast w Polsce powinna wyjść ~1800–2600 HDD (samotest pilnuje tego zakresu dla Opola).
4. Commit obu plików. Select miasta w M02 zasila się z `Object.keys(P.CLIMATE)` — HTML bez zmian.

**TLS na tej maszynie:** lokalna inspekcja TLS (proxy/AV) sprawia, że `fetch` w Node kończy się `UNABLE_TO_VERIFY_LEAF_SIGNATURE`. Generator ma cache plikowy `tools/cache/` (gitignorowany) — przy błędzie pobierania wypisuje gotową komendę `Invoke-WebRequest … -OutFile <plik cache>`; po jej wykonaniu w PowerShellu (ufa magazynowi Windows) generator działa z cache.
...
**Jednostka łączna** — select budynku M04 ma dodatkową pozycję „Łącznie (wszystkie budynki)" (sentinel `P.MERGED = '__laczne__'`, etykieta `P.MERGED_LABEL`, helper `P.unitLabel(b)` — wszystko w [js/kz.config.js](js/kz.config.js)): `simulate(P.MERGED, med)` bilansuje **sumę wszystkich budynków z M01**. Tryb to po prostu `state.m04Building === P.MERGED` (bez osobnego pola stanu; stary JSON wczytuje się bez zmian). Mechanika:
- **Koszt i driver** — `P.monthCell(P.MERGED, …)` ma gałąź sentinela: suma per-budynkowych komórek (fakt lub prognoza liczona **osobno per budynek** — rwane zakresy danych nie psują sumy; status `forecast` gdy choć jedna składowa jest prognozą, `none` gdy żaden budynek nic nie wnosi). Driver = `P.getArea(P.MERGED)` (Σ powierzchni, gałąź w [js/kz.data.js](js/kz.data.js)) lub Σ wody (qty zsumowanej komórki).
- **Część ustalona** — M03 zostaje per budynek; w `simulate` liczona jako **kwoty per miesiąc** (`fixedAmt[i]` = Σ po budynkach: stawka budynku z carry-forward × jego driver). Granica „ogona" = ostatnia stawka wpisana dla **któregokolwiek** budynku. `rate[]` miesięcy ustalonych to stawka **implikowana** (kwota / driver łączny) — tylko do wykresów `rate_*`; dla pojedynczego budynku ścieżka liczy dokładnie to co dawniej.
- **Guardy** — nazwa budynku nie może kolidować z sentinelem (`m01AddBuilding`/`m01RenameBuilding` zwracają `false`); sentinel bez `|` (format kluczy `P.advances`); `P.m04Building()` zachowuje wybór sentinela bez nadpisywania stanu. M01–M03 i M02 bez zmian (sentinel żyje tylko w M04). Testy w sekcji 7 samotestu.

Kontrolki M04:
- **Budynek** (`#kz-m04-building`) — własny select, jak w M02, z dodatkową ostatnią pozycją `P.MERGED_LABEL` (jednostka łączna; opcja dopinana w `fillBuildingSelect`, HTML bez zmian).
...
- Klucze: cena i temperatura `"RRRR-MM"`, zaliczka `"budynek|medium|RRRR-MM"`, miesiąc bezwzględny `P.absM(y,m)` do porównań/okresów.

## Uwaga o wersji

Wersja `1.2.0` jest zapisana w **czterech** miejscach, które trzeba synchronizować przy bumpie: `P.VERSION` w [js/kz.config.js](js/kz.config.js), nagłówek/stopka w [kalkulator-zaliczek.v1.2.html](kalkulator-zaliczek.v1.2.html), tytuł w README oraz **nazwa pliku HTML** (`…v1.2.html` — odzwierciedla wersję `major.minor`; przy bumpie zmienić nazwę i wszystkie linki do niej w README/CLAUDE). Uwaga: zmiana nazwy łamie istniejące zakładki/linki do starej nazwy.
